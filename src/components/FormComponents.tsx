...
// Label component — accepts htmlFor for WCAG Level A compliance
interface LblProps {
  children: React.ReactNode;
  req?: boolean;
  htmlFor?: string;
...
// Form Field wrapper with error support and label-input association
interface FFProps {
  label: string;
  /** Unique field ID used for htmlFor/id association. Falls back to label slug. */
  fieldId?: string;
  req?: boolean;
  hint?: string;
  span?: number;
...
/** Generate a stable DOM id from a label string */
...
export const FF: React.FC<FFProps> = ({ label, fieldId, req, hint, span, error, children }) => {
  ...
  return (
    <div style={{
      ...
      {/* Clone the first child element to inject the id for label association */}
